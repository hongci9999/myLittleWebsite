...
- **파일 구조형 패턴 추상화** - 학습자료 등 계층적 드릴다운 UI 재사용
  - `shared/config/file-structure.ts`: 타입, 레지스트리, breadcrumb 해석
  - `file-structure-sections/`: 섹션별 config (learning-info-engineer 등)
  - `pages/FileStructure/`: 공통 Top/Category/DocList/DocViewer 페이지
  - `.cursor/rules/file-structure-pattern.mdc`: 패턴 룰 (일관적 개발용)
