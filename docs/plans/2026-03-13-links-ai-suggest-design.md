...
| 순서 | 할 일 |
|------|-------|
| 1 | Ollama 설치, `ollama pull phi3:mini` |
| 2 | 서버 `POST /api/links/ai-suggest` 엔드포인트 추가 |
| 3 | LinksAdminPage에 "AI로 채우기" 버튼, suggestedLabels → valueIds 매칭 |
| 4 | (선택) 웹사이트 읽기: cheerio로 HTML 파싱 |
| 5 | (선택) 동영상 요약: YouTube transcript API |
