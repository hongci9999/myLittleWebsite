...
날짜: 2026-04-04 (§8 공개 메타, 유튜브 자막·Gemini 연동 반영)
...
AI 제목·설명·분류 추천. (`AI_TEXT_PROVIDER`에 따라 Ollama 또는 Gemini)
...
```json
{
  "ai": {
    "mode": "local",
    "label": "Ollama · lfm2:24b @ localhost:11434"
  }
}
```
...
| 필드 | 설명 |
|------|------|
| `ai.mode` | `"api"` — Gemini 등 클라우드 API 경로 / `"local"` — Ollama 등 로컬 |
| `ai.label` | 모델·호스트 힌트가 들어 있는 짧은 문자열(환경 변수 기준) |
