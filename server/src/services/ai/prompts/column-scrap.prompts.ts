...
{"title":"","summary":"","bodyMd":"","sourceKind":"x","tags":[]}`
  ...
  /** Obsidian Web Clipper 붙여넣기 — 붙여넣은 노트·자막만 분석 */
  deepAnalysisNoteObsidianClip(pastedClip: string): string {
    return `역할: 웹 스크랩용 **심층 분석 노트** 작성. 아래는 사용자가 Obsidian Web Clipper에서 **붙여넣은 노트 전문**입니다. 한국어로만 답하세요.

**반드시 지킬 것**
- 아래 붙여넣은 텍스트만 분석 근거로 사용하세요. **## 트랜스크립트**(또는 타임스탬프 자막 블록)가 있으면 그것을 최우선으로 읽으세요.
- 영상·음성을 직접 본다고 가정하지 마세요. URL 접속·외부 검색·일반 지식으로 내용을 채우지 마세요.
- 붙여넣은 텍스트에 없는 내용은 쓰지 말고, 불가피하면 [추측]으로만 표시하세요.

다음을 빠짐없이 채우세요. 총 분량 **800~1800자** 권장(짧게 끊지 말 것).

1) **주제 한 줄**: 붙여넣은 텍스트 기준 이 클립이 무엇에 대한 것인지
2) **핵심 내용**: 자막·본문에 나온 주장, 절차, 설정값, 버전, 수치, 인용 등 **구체적 사실**
3) **구조**: 자막·본문이 어떤 순서로 전개되는지(섹션·단계)
4) **대상**: 누가 보면·읽으면 좋은지(텍스트 근거 범위 안에서)
5) **활용**: 실무·학습에서 어떻게 쓸 수 있는지
6) **전제·한계**: 자막·설명에 밝힌 제한, 주의, 텍스트만으로 확인 불가한 부분

자막·본문이 비어 있거나 의미 없으면 그 사실을 명시하세요.
