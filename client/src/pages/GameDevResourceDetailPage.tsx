...
export default function GameDevResourceDetailPage() {
  const { slug: slugParam } = useParams<{ slug: string }>()
  const { token } = useAuth()
  const { openGameDevAdmin } = useGameDevAdmin()
  const [scrap, setScrap] = useState<GameDevResource | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setDbOff(false)
      try {
        const s = await fetchGameDevResourceBySlug(slugParam)
    ...
    return (
      <div className="mx-auto max-w-3xl px-5 py-12 text-center sm:px-8 md:px-10 md:py-16">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Supabase와{' '}
          <code className="rounded bg-muted px-1 font-mono text-xs">game_dev_resources</code> 마이그레이션을
          확인하세요.
        </p>
        <Button asChild className="mt-6" variant="outline">
          <Link to="/game-dev">목록으로</Link>
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 pb-14 pt-6 sm:px-8 md:px-10 md:pb-20 md:pt-8">
      <div className="flex flex-wrap gap-2">
        ...
        {token ? (
          <Button
            ...
            size="sm"
            className="h-8"
            onClick={() => openGameDevAdmin({ slug: s.slug })}
          >
            편집
          </Button>
        ) : null}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs font-semibold uppercase tracking-[0.2em] text-primary/85">
      ...
      {s.bodyMd?.trim() ? (
        ...
        <p className="mt-12 text-sm text-muted-foreground">
          상세 본문이 비어 있습니다. 목록 페이지 상단의 자료 추가·편집에서 마크다운을 넣을 수 있습니다.
        </p>
      )}
