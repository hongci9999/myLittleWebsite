...
- **링크 등록 시**: **URL만** 입력해도 AI가 `title`, `description`, `valueIds`(분류)를 자동 추천
...
```bash
ollama pull lfm2:24b       # 기본 사용 모델 (24B MoE, 14GB)
ollama pull phi3:mini      # 대안: 작은 모델, 4GB RAM
...
- **URL만** 있어도 AI가 제목·설명·분류 생성 (title은 선택, 있으면 우선 사용)
...
JSON으로 반환: { "title": "사이트 제목", "description": "한 줄 설명", "suggestedLabels": ["디자인", "웹서비스"] }
```

### 5.4 UX

- 링크 폼에 "AI로 채우기" 버튼 (URL만 있어도 실행)
- **진행 상태 시각화**: URL 전송 → AI 분석 → 완료
- AI 추천 → 폼에 채움 → 사용자가 수정·확인 후 저장
...
- **Request**: `url` 필수, `title` 선택 (있으면 AI가 우선 사용)
- **Response**: `{ title, description, valueIds }`
...
| 2 | (기존 custom 태그 있으면) `docs/plans/2026-03-13-migrate-custom-tags-to-purpose.sql` 실행 |
...
- **서버**: POST /api/links/ai-suggest — Ollama 호출, `title`·`description`·`valueIds` 반환. 기존 태그 매칭 또는 custom dimension에 새 태그 생성
- **웹사이트 읽기**: cheerio로 HTML 파싱, 2단계 AI (1단계 사이트 분석 → 2단계 메타데이터 생성). fetch 실패 시 URL만 폴백
- **태그 목적/종류 분류**: 모든 태그는 목적(purpose) 또는 종류(medium)로만. AI suggest·수동 추가 모두 dimension 지정
- **custom→종류 마이그레이션**: 기존 custom 축 태그 이동 시 `docs/plans/2026-03-13-migrate-custom-tags-to-purpose.sql`
- **LinkForm** (widgets/LinkForm): 링크 추가·수정 공통 폼. URL만 입력 → AI로 채우기 → 진행 상태(URL 전송→AI 분석→완료) 시각화
- **AddLinkDialog**: Dialog + LinkForm. LinksPage 검색창 오른쪽 "추가" 버튼, LinksAdminPage "새 링크 추가" 버튼
- **LinksAdminPage**: LinkForm으로 수정 폼 통합 (추가 시 AddLinkDialog, 수정 시 인라인 LinkForm)

---

## 11. 참고

- [decisions/0012-ollama-ai-links.md](../decisions/0012-ollama-ai-links.md) — Ollama 서버 방식 채택 ADR
...
- [api-spec.md](../api-spec.md) — POST /api/links/ai-suggest 명세
