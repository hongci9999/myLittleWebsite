...
import {
  isSourceKind as isAiScrapSourceKind,
  type SourceKind as AiScrapSourceKind,
...
function inferAiToolSourceKindFromUrl(urlStr: string): AiScrapSourceKind {
  try {
    const u = new URL(urlStr.trim())
    const host = u.hostname.replace(/^www\./, '').toLowerCase()
    const path = u.pathname.toLowerCase()
...
async function expandAiToolBodyMarkdown(params: {
  url: string
  title: string
  summaryLine: string
  siteAnalysis: string
  ...
  const { url, title, summaryLine, siteAnalysis } = params
  const prompt = `아래 [심층 분석 노트]만 참고해 **AI 개발 도구 스크랩**용 마크다운 본문을 작성하세요. JSON 금지, 마크다운 원문만 출력.
 ...
 * X/트위터는 HTML fetch 생략(칼럼 스크랩과 동일 이유).
 */
export async function suggestAiToolScrapFromUrl(url: string): Promise<AiToolScrapAiFillResult> {
  ...
  if (skipHtml) {
    const handle = xStatusHandleFromUrl(trimmed)
    const handleLine = handle ? `계정 @${handle} 로 추정됩니다.` : '계정명은 URL에서 추정하지 못했습니다.'
    const xPrompt = `다음은 X(트위터) 링크입니다. **AI 개발 도구 스크랩**용으로 JSON 한 개만 출력하세요. HTML·오류 문구를 title·summary·bodyMd에 넣지 마세요.
...
{"title":"","summary":"","bodyMd":"","sourceKind":"other","tags":[]}`
  ...
  if (content && content.fullText.trim().length > 40) {
    const analyzePrompt = `역할: **AI 개발 도구** 관점의 심층 분석 노트. 페이지 텍스트입니다. 한국어로만.
...
{"title":"","summary":"","bodyMd":"## 요약\\n\\n...\\n\\n## 상세 정리\\n\\n...","sourceKind":"doc","tags":[]}`
  ...
  let sourceKind: AiScrapSourceKind = kindHint
  ...
  if (siteAnalysis.length >= 400 && bodyMd.length < 500) {
    try {
      const expanded = await expandAiToolBodyMarkdown({
        url: trimmed,
        title,
        summaryLine: summaryForExpand,
        siteAnalysis,
